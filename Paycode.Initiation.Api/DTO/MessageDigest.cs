...
namespace Paycode.Initiation.Api.DTO
{
    public abstract class MessageDigest
    {
        ...
        public static MessageDigest GetInstance(string algorithm)
        {
            ...
            string lower = algorithm.Replace("-", "").ToLower();
            if (lower != null)
            {
                if (lower == "sha1")
                {
                    messageDigest = new MessageDigest<SHA1Managed>();
                }
                else if (lower == "md5")
                {
                    messageDigest = new MessageDigest<MD5CryptoServiceProvider>();
                }
                else
                {
                    if (lower != "sha512")
                    ...
                    messageDigest = new MessageDigest<SHA512CryptoServiceProvider>();
                }
    ...
    public class MessageDigest<TAlgorithm> : MessageDigest
    where TAlgorithm : HashAlgorithm, new()
    {
        ...
        {
            this._stream.Write(input, 0, (int)input.Length);
        }
